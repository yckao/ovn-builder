...
(($# == 2)) || die "usage: validate-stage-metadata.sh METADATA_DIR OUTPUT_JSON"
...
[[ $IMAGE_PREFIX == "$expected_prefix" ]] \
    || die "verified release prefix must be $expected_prefix"
...
' .github/ci-matrix.json >/dev/null \
    || die "release staging supports only the locked v1 product and builder cells"
...
for file in "${metadata_files[@]}"; do
    ...
    jq -e \
        ...
        --argjson expected_count "$expected_count" '
          ...
          .schema == "ovn-builder.release-stage.v1" and
    ...
    --arg stage_base "$stage_base" '
      ([.cells[] as $cell |
        ({
          ...
          canonical_ref: ($prefix + "/" + $cell.product + "-debs:" + $cell.release_tag + "-" + $sha)
        }, {
          cell_id: $cell.id,
          kind: "runtime",
          source_ref: ($prefix + "/" + $cell.product + ":" + $stage_base + "-" + $cell.id),
          canonical_ref: ($prefix + "/" + $cell.product + ":" + $cell.release_tag + "-" + $sha)
        }, {
          cell_id: $cell.id,
          kind: "oci-bundle",
          source_ref: ($prefix + "/" + $cell.product + "-debs-oci:" + $stage_base + "-" + $cell.id),
          canonical_ref: ($prefix + "/" + $cell.product + "-debs-oci:" + $cell.release_tag + "-" + $sha)
        })] +
       [.builders[] as $cell | {
          cell_id: $cell.id,
          kind: "builder",
          source_ref: ($prefix + "/builder:" + $stage_base + "-builder-" + $cell.id),
          canonical_ref: ($prefix + "/builder:" + $cell.release_tag + "-" + $sha)
...
' <<< "$actual" >/dev/null || die "invalid or duplicate staged descriptors"

actual_contract=$(jq -cS '[.[] | del(.digest)]' <<< "$actual")
[[ $actual_contract == "$expected" ]] \
    || die "staging metadata does not match the locked release reference contract"
...
jq -nS \
    ...
    --argjson descriptors "$actual" '{
      schema: "ovn-builder.release-set.v1",
      image_prefix: $prefix,
      release_run: {
        id: $run_id,
        attempt: $run_attempt,
...
echo "validated 14 staged descriptors and wrote $output_json"

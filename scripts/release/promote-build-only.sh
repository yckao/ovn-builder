...
(($# == 1)) || die "usage: promote-build-only.sh RELEASE_SET_JSON"
release_set=$1
...
jq -e \
  ...
  .schema == "ovn-builder.build-only-release-set.v1" and
  ...
  all(.descriptors[];
    ...
    (.source_ref | contains(":_staging-build-only-" + $root.release_run.id + "-")) and
    (.canonical_ref | startswith($prefix + "/")) and
    (.canonical_ref | endswith("-kernel-unverified-" + $root.release_run.sha)) and
...
' "$release_set" >/dev/null || die "invalid kernel-unverified release set"
...
# Do not write a canonical tag until every staging digest and existing
# build-only canonical tag has passed this read-only preflight.
...
echo "all kernel-unverified staged digests and canonical tags passed preflight"
...
echo "verified all 14 canonical kernel-unverified full-SHA tags"

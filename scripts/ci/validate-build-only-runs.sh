...
(($# == 2)) || die "usage: validate-build-only-runs.sh CI_RUN_ID REPRO_RUN_ID"
...
[[ $GITHUB_REF == refs/heads/main ]] || die "build-only publication must run from main"
...
' .github/ci-matrix.json >/dev/null \
    || die "build-only publication requires the exact locked v1 matrices"
...
assert_run() {
    ...
    jq -e \
        ...
        --arg expected_path "$expected_path" '
          ...
          .run_attempt >= 1
        ' "$file" >/dev/null \
        || die "$description is not a successful same-SHA main workflow_dispatch run"
}
...
assert_exact_job_set() {
    ...
    ' "$file" >/dev/null \
        || die "$description job set is missing, duplicated, unexpected, or unsuccessful"
}

assert_build_only_ci_jobs() {
    local file=$1
    local required=$2

    jq -e --argjson required "$required" '
      def is_required: . as $name | ($required | index($name)) != null;
      def is_kernel_job:
        . == "exact-kernel" or startswith("exact-kernel /");
      ([.jobs[] | select(.name | is_required) | .name] | sort) == ($required | sort) and
      ([.jobs[] | select(.name | is_required)] | length) == ($required | length) and
      all(.jobs[];
        if (.name | is_required) then
          .status == "completed" and .conclusion == "success"
        else
          (.name | is_kernel_job) and
          .status == "completed" and .conclusion == "skipped"
        end) and
      ([.jobs[] | select(.name | is_kernel_job)] | length) <= 2
    ' "$file" >/dev/null \
        || die "CI must contain the seven successful build-only jobs and no executed kernel job"
}
...
[[ $(jq 'length' <<< "$ci_required_jobs") == 7 ]] \
    || die "internal build-only CI contract is not seven jobs"
[[ $(jq 'length' <<< "$repro_jobs") == 13 ]] \
    || die "internal reproducibility job contract is not thirteen jobs"

assert_build_only_ci_jobs "$tmp/ci-jobs.json" "$ci_required_jobs"
